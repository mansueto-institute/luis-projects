...
states=['Kerala','Telengana','Delhi','Uttar Pradesh','Haryana','Ladakh','Tamil Nadu','Karnataka','Maharashtra','Punjab','Jammu and Kashmir','Andhra Pradesh','Odisha','Uttarakhand','Puducherry','West Bengal','Chhattisgarh','Gujarat','Himachal Pradesh','Madhya Pradesh','Chandigarh','Bihar','Manipur','Mizoram','Goa','Andaman and Nicobar Islands','Jharkhand','Arunachal Pradesh','Assam','Rajasthan']
...
#lyyy=np.cumsum(lwy)
    ...
    plt.plot(date,DeltaT,'ro',ms=6,alpha=.4,label=r'Projection w/ $R_t =R_{\rm today}$')
    plt.fill_between(date,upp, lpp,color='red', alpha=0.1)
    plt.plot( (xd[-1],xd[-1]),(0.,max(DeltaT)+20.),'k--')
    plt.text(xd[-1]+timedelta(days=1),max(DeltaT), 'today',fontsize=14)
    ...
    fig.autofmt_xdate()
    plt.tight_layout()
    str='images/Observed_Predicted_New_Cases_No_Control_'+state+'.png'
    plt.savefig(str)
    #plt.show()
